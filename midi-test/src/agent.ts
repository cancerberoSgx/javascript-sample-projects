...
export class AgentPlayer{
  agents: Agent[] = []
  constructor(public player: IWebAudioFontPlayer, public audioContext: AudioContext, public preset: Preset){}
  playCompasses(compassCount: number){
    this.agents.map(agent=>agent.buildCompassNotes(compassCount)).flat()
        .forEach(n => {
          this.player.queueChord(this.audioContext, this.audioContext.destination, this.preset, n.t, n.notes, n.duration, n.volume)
        })
  }
}

const NOTES = [0, 0, 2, 4, 5, 7, 9, 11]
function note(n:number,octave=0){
...
export class Waltz2RightAgent extends Agent {
  ...
  buildCompass(t0: number): Note[] {
    ...
    return [
      { t: 0, notes: [note(3)], volume: 1.0, duration: unit },
      { t: 1, notes: [note(2), note(5)], volume: 0.5, duration: unit*0.5 },
      { t: 1.5, notes: [note(4)], volume: 0.8, duration: unit*0.5 },
      { t: 2, notes: [note(7)], volume: 0.8, duration: unit },
      { t: 3, notes: [note(5, -1)], volume: 1.0, duration: unit },
      { t: 4, notes: [note(2), note(4)], volume: 0.5, duration: unit },
      { t: 5, notes: [note(4), note(7)], volume: 0.9, duration: unit },
...
export class Waltz2LeftAgent extends Agent {
  ...
  buildCompass(t0: number): Note[] {
    ...
    return [
      ...
      { t: 2, notes: [note(3)], volume: 0.2, duration: unit },
