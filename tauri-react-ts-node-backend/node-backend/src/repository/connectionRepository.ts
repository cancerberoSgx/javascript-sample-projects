...
export function createConnection(profileId: number, input: ConnectionInput): Connection {
  ...
  const result = db.prepare(`
    INSERT INTO connections (profile_id, type, name, db_host, db_port, db_name, db_user, db_password)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run(profileId, input.type, input.name, input.db_host, input.db_port, input.db_name, input.db_user, input.db_password);
...
export function updateConnection(id: number, profileId: number, input: ConnectionInput): Connection | null {
  ...
  db.prepare(`
    UPDATE connections
    SET type = ?, name = ?, db_host = ?, db_port = ?, db_name = ?, db_user = ?, db_password = ?
    WHERE id = ? AND profile_id = ?
  `).run(input.type, input.name, input.db_host, input.db_port, input.db_name, input.db_user, input.db_password, id, profileId);
